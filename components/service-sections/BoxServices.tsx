...
export default function BoxServices(props: MyProps) {
  ...
  return (
    <section className="p-4 mt-20 overflow-hidden">
      <div className="text-center mb-5">
        ...
        <a href={featuredSection?.linkUrl} className=""><button className=" bg-[#00C2CC] hover:bg-[#001B42] py-2 rounded text-white font-bold lg:w-[250px] flex justify-center uppercase"><span className="text-center px-1">{featuredSection?.linkText}</span></button></a>
        </div>
      <div>
        {featuredSection?.featuredData.map((item, index) => (
          <div
            key={index}
            className={`flex flex-col bg-gray-200 rounded p-5 mt-5 ${ index % 2 == 0 ? 'md:flex-row' : 'md:flex-row-reverse'} items-center mb-8`}
          >
